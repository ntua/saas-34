    ...
    public render() {
        return (
            <Link to={ `/question/${this.props.question._id}` } className="search-result">
                <Card className="my-5">
                    <Card.Header>Ρωτήθηκε πρι από {new Date(this.state.question.timestamp * 1000).toISOString().split('T')[0]}</Card.Header>
                    <Card.Body>
                        ...
                        <Row className="mt-3">
                            ...
                            <Col xs={ 3 } className="card-text text-right">
                                Δημιουργήθηκε από { this.state.question.user.username }
                            </Col>
                        </Row>
                    </Card.Body>
                </Card>
