...
exports.loadComments = async (req, res, next, id) => {
  try {
    ...
    if (!comment) return res.status(404).json({ message: 'Το σχόλιο δεν βρέθηκε' });
    req.comment = comment;
  } catch (error) {
    if (error.name === 'CastError') return res.status(400).json({ message: 'Λανθασμένο id σχολίου.' });
    return next(error);
  }
  next();
};
...
exports.validate = [
  body('Σχόλιο')
    ...
    .notEmpty()
    .withMessage('Δεν μπορεί να είναι κενό')
